...
export default function Process() {
  return (
    <Section id="process" tone="darkAlt" labelledBy="process-title">
      <SectionHeader
        id="process-title"
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div
          aria-hidden="true"
          className="absolute left-0 top-8 hidden h-px w-full bg-white/15 lg:block"
        />
        <motion.div
          aria-hidden="true"
          initial={{ scaleX: 0 }}
          ...
          className="absolute left-0 top-8 hidden h-px w-full origin-left bg-acid-400 lg:block"
        />
        ...
        >
          {steps.map((step) => (
            ...
            >
              {/* tone outlineDark tem fundo opaco de propósito: sem ele o
                  trilho da timeline aparece por trás do número. */}
              <IconFrame size="lg" tone="outlineDark" className="relative z-10">
                {step.number}
              </IconFrame>
              <h3 className="mb-2 mt-5 font-bold text-on-dark">{step.title}</h3>
